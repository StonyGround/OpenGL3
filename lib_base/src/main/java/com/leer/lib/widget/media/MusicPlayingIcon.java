...
public class MusicPlayingIcon extends View {
    ...
    private float[] initHigh = {0.5f, 1.0f, 0.7f, 0.3f};
    ...
    public MusicPlayingIcon(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MusicPlayingIcon(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MusicPlayingIcon);
        pointerColor = ta.getColor(R.styleable.MusicPlayingIcon_pointer_color,
                getResources().getColor(R.color.text_right));
        stopColor = ta.getColor(R.styleable.MusicPlayingIcon_stop_color,
                getResources().getColor(R.color.text_right));
        pointerNum = ta.getInt(R.styleable.MusicPlayingIcon_pointer_num, 4);
        pointerWidth = ta.getDimensionPixelOffset(R.styleable.MusicPlayingIcon_pointer_width,
                ConvertUtils.dp2px(1));
        pointerSpeed = ta.getInt(R.styleable.MusicPlayingIcon_pointer_speed, 40);
        ta.recycle();
        init();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        pointerPadding =
                (getWidth() - getPaddingLeft() - getPaddingRight() - pointerWidth * pointerNum) / (pointerNum - 1);
    }
...
//        paint.setColor(stopColor);
        ...
        for (int i = 0; i < pointerNum; i++) {
            ...
            pointer =
                    new Pointer(initHigh[i] * (getHeight() - getPaddingBottom() - getPaddingTop()));
