...
public class PullToRefreshLayout extends FrameLayout {
    ...
    private void cal() {
        head_height = SizeUtils.dp2px(HEAD_HEIGHT);
        foot_height = SizeUtils.dp2px(FOOT_HEIGHT);
        head_height_2 = SizeUtils.dp2px(HEAD_HEIGHT * 2);
        foot_height_2 = SizeUtils.dp2px(FOOT_HEIGHT * 2);
...
//            mHeaderView = new RefreshView(getContext());
            mHeaderView = new RefreshViewHoolink(getContext());
        } else {
...
//            mFooterView = new RefreshView(getContext());
            mFooterView = new RefreshViewHoolink(getContext());
                ...
                if (dy1 > 0 && canRefresh) {
                    ...
                    if (canLoadMore) {
                        if (Math.abs(dy1) >= foot_height) {
                            createAnimatorTranslationY(State.LOAD_MORE,
                                    Math.abs(dy1) > foot_height_2 ? foot_height_2
                                            : Math.abs(dy1), foot_height, () -> {
                                        isLoadMore = true;
                                        if (refreshListener != null) {
    ...
    public void setHeadHeight(int dp) {
        head_height = SizeUtils.dp2px(dp);
    }
    ...
    public void setFootHeight(int dp) {
        foot_height = SizeUtils.dp2px(dp);
    }
    ...
    public void setAllHeight(int refresh, int loadMore) {
        head_height = SizeUtils.dp2px(refresh);
        foot_height = SizeUtils.dp2px(loadMore);
    }
    ...
    public void setMaxHeadHeight(int dp) {
        if (head_height >= SizeUtils.dp2px(dp)) {
            return;
        }
        head_height_2 = SizeUtils.dp2px(dp);
    }
    ...
    public void setMaxFootHeight(int dp) {
        if (foot_height >= SizeUtils.dp2px(dp)) {
            return;
        }
        foot_height_2 = SizeUtils.dp2px(dp);
    }
    ...
    public void setAllMaxHeight(int refresh, int loadMore) {
        if (head_height >= SizeUtils.dp2px(refresh)) {
            return;
        }
        if (foot_height >= SizeUtils.dp2px(loadMore)) {
            return;
        }
        head_height_2 = SizeUtils.dp2px(refresh);
        foot_height_2 = SizeUtils.dp2px(loadMore);
    }
